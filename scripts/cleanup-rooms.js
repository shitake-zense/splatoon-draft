...
async function cleanup() {
  ...
  for (const roomId of roomIds) {
    ...
    // lobby.members の最も古い joinedAt を作成日時とみなす
    const members = room?.lobby?.members || {};
    const joinedAts = Object.values(members).map(m => m.joinedAt || 0);
    const createdAt = joinedAts.length > 0 ? Math.min(...joinedAts) : 0;

    if (createdAt === 0 || now - createdAt > EXPIRE_MS) {
