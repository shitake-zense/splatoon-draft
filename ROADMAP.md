...
### 🐞 2026-07-28 実機バグ修正（オーナー報告・8人/個人ピック）

#### ① 代表者の選ばれ方が壊れていた（1チームに2人・もう1チームに0人）
- **原因**: `buildAndStartDraft` の代表者解決が「**α代表＝ホスト（`myMemberId`）／β代表＝bravoの非ホスト最古参**」という *ホスト＝アルファ前提* の実装だった。ホストがブラボー所属だと α代表にブラボー所属のホストが入り、**ブラボーに代表者2人・アルファに0人**になる。さらに代表者BANの送信経路（`confirmBans` の `pending_alpha_bans`/`pending_bravo_bans` 振り分け、`renderBanPanel` の自チーム判定、`renderGrid`/`pickWeapon` の `it`、タイマー自動補完）も同じ前提で書かれており、アルファ側が**BANを送信できない**状態だった。
- **修正（仕様変更＝オーナー指示）**: **代表者＝各チームの「1ピック目」**（`pickOrderAlpha/Bravo` の先頭にいるそのチームのメンバー／ピック順が無いモードは `joinedAt` 最古）。純粋関数 `resolveRepresentative(team, members, order)` に切り出し、`buildAndStartDraft` で **両チームとも同じ規則**で解決。
  - 参照側は `repIdOf(S,team)`（未設定の古い state はその場で復元＝後方互換）／`repNameOf(S,team)`／`myTeamOf(S)`（**所属は `S.members` が真実**。ホスト判定は members に自分がいない場合のみのフォールバック）に統一。
  - ホスト＝アルファ前提を除去した箇所: `isRepBanRepresentative`／`gachaCanOperate`／`isTradeOperator`／`isSeriesChooserOperator`／`mySimPickTeam` フォールバック／`renderBanPanel`（自チーム判定・hint・送信ボタン）／`confirmBans`（`pending_${myTeam}_bans` へ送信）／`renderGrid`（`repIt`/`it`/チームピックの clickable）／`pickWeapon` の BAN 分岐／`handleTimeoutSelf`（自チーム分を送信）／`handleTimeout`（**両チームとも Firebase を確認して未送信のみ補完**）。
- **UI**: BANパネルに **`#ban-reps`＝両チームの代表者名**を常時表示（自分が担当なら「あなた」バッジ）。BAN中のプロンプトも「あなたが代表者」/「◯◯ が選択中です」に。CSS `.ban-reps`/`.rep-chip`/`.rep-you`。

#### ② 連戦「負けチームがルール設定」が機能せず担当者も不明だった
- **原因A**: ①の代表者バグで敗者チームの代表者が誤っていた（担当者が存在しない／他チームにいた）。
- **原因B**: ホストの `subscribeSeriesRule` が `sr.ruleChooser==='bravo'` のシグナルしか受け付けず、**敗者がアルファでその代表者が非ホストだと永久に進まなかった**（→ `awaitingRule` なら両チーム受理に変更）。
- **原因C**: 表示が**チーム名だけ**で「誰が選ぶのか」が一切分からなかった。
- **修正**: 結果画面の series-area に **🎯 担当者バナー**（敗者チーム名＋代表者名＋自分なら「あなた」バッジ）を追加。勝敗入力の**前**にも「次のルールは負けたチームの代表者が選びます — α: ◯◯ ／ β: ◯◯」を表示。さらに**担当者が操作できない場合のホスト代理決定**（ホストにもルール4ボタン＋注記）を追加して進行が詰まらないようにした。CSS `.series-who`/`.who-name`/`.series-proxy`。
- **検証**: `node --check` OK／**単体71件パス**（新規 `tests/representative.test.js` 11件＝1ピック目優先・ホストがブラボーでも各チーム1人・フォールバック・決定性、`tests/operator-role.test.js` 11件＝BAN/ガチャ/トレード/敗者ルール選択の操作可否をホスト＝ブラボー構成で検証。テストharnessに `isMulti/isHost/myMemberId` の注入を追加）。**ローカル実機（ソロ）で BO3＋負けチームルール設定を E2E**（第1試合→勝者入力→ルール選択UI→ガチホコ選択→第2試合がガチホコで開始まで確認）。
